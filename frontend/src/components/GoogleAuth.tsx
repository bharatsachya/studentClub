...
const GoogleAuth: React.FC<GoogleAuthProps> = ({
  ...
  const initializeGoogle = () => {
    if (window.google && GOOGLE_CLIENT_ID) {
      ...
      window.googleAuthCallback = handleGoogleResponse;
  ...
  const handleGoogleResponse = async (response: any) => {
    ...
    try {
      const backendResponse = await axios.post(
        `http://${BACKEND_API_URL}/api/v1/auth/google/verify`,
        {
      ...
      onSuccess?.(backendResponse.data.data.user);
  ...
  const handleGoogleSignIn = () => {
    if (!isGoogleLoaded || !window.google) {
      onError?.('Google Sign-In is not loaded yet. Please try again.');
      return;
    }
      ...
      callback: async (response: any) => {
        if (response.access_token) {
          try {
            ...
            // For this flow, we'd need to handle it differently
            // For now, we'll use the ID token flow which is more secure
  ...
  return (
    <div className={`w-full ${className}`}>
      {/* Hidden button for Google rendering */}
